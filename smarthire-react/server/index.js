...
app.use((req, res, next) => {
  ...
  if (proto && proto === 'http') {
    return res.redirect(301, `https://${req.headers.host || 'smarthireus.com'}${req.url}`);
  }
  next();
})
...
app.get('/api/candidates', authenticateToken, async (req, res) => {
  ...
  const normalized = (candidatesStore || []).map(c => ({
    ...
    experience: extractCandidateExperience(c.resumeText, c.summary, c.extracted_profile, c.role || c.job_title),
    status: c.status || 'New',
  }));
...
app.post('/api/screening/public-submit', async (req, res) => {
  ...
  const newSession = {
    sessionId,
    jobId: job.id,
    targetReqId: job.reqId || job.id,
    reqId: job.reqId || job.id,
    jobReqId: job.reqId || job.id,
    ...
    chatHistory: [
      {
        role: 'assistant',
        content: `Hi ${finalCandidateName}, thank you for submitting your direct candidate application for ${job.title}. Your profile and resume have been automatically analyzed by AI.`
      },
      {
        role: 'user',
  ...
  try {
    ...
    const candidateDoc = {
      ...
      matchedJobClient: job.client || job.source || 'COOLSOFT',
      matchedJobTitle: job.title,
      client: job.client || job.source || 'COOLSOFT',
      name: finalCandidateName,
  ...
  try {
    ...
    const newSession = {
      ...
      chatHistory: [
        {
          role: 'assistant',
          content: `Hi ${finalCandidateName}, thank you for applying for ${job.title}. Your profile and resume file have been saved.`
        }
      ],
      screeningComplete: true,
      conversationStage: 'submitted'
  ...
  try {
    ...
    if (isMongoConnected) {
      try {
        ...
        user = await RecruiterDoc.findOne({ email: emailRegex });
  ...
  try {
    ...
    if (isMongoConnected) {
      try {
        ...
        const mongoEmails = new Set(recruiters.map(r => (r.email || '').toLowerCase().trim()));
        const diskExtra = recruitersMock.filter(m => !mongoEmails.has((m.email || '').toLowerCase().trim()));
        ...
        console.warn('MongoDB recruiters list fallback to disk:', mErr.message);
      }
    }
...
// GET /api/analytics/recruiter-leaderboard — Recruiter KPI & Team Performance Leaderboard
app.get(['/api/analytics/recruiter-leaderboard', '/api/recruiter-leaderboard'], (req, res) => {
  try {
    ...
    const roster = (recruitersMock && recruitersMock.length > 0) ? recruitersMock : [
      ...
      { id: 'rec-2', name: 'Vaibhav Bisen', email: 'vaibhav@coolsofttech.com', role: 'Lead Recruiter' },
      { id: 'rec-3', name: 'Sukamal Chatterjee', email: 'kamal@coolsofttech.com', role: 'Senior Recruiter' },
      { id: 'rec-4', name: 'Gourav', email: 'gourav@coolsofttech.com', role: 'Recruiter' },
      { id: 'rec-5', name: 'Naveen Bhardwaj', email: 'naveen@coolsofttech.com', role: 'Recruiter' },
      { id: 'rec-6', name: 'Rahul Sharma', email: 'rahul@coolsofttech.com', role: 'Recruiter' },
      { id: 'rec-7', name: 'Priya Verma', email: 'priya@coolsofttech.com', role: 'Recruiter' },
      { id: 'rec-8', name: 'Prudhvi Sevveti', email: 'prudhvi.s@smarthire.com', role: 'Recruiter' },
      { id: 'rec-9', name: 'Nitin Bhosale', email: 'nitin.b@smarthire.com', role: 'Recruiter' },
      { id: 'rec-10', name: 'Naveen Korimelli', email: 'naveen.k@smarthire.com', role: 'Recruiter' },
      { id: 'rec-11', name: 'Ajay Arya', email: 'ajay.a@smarthire.com', role: 'Recruiter' },
      { id: 'rec-12', name: 'Raj Barve', email: 'raj.b@smarthire.com', role: 'Recruiter' },
      { id: 'rec-13', name: 'Pankaj Maharwade', email: 'pankaj.m@smarthire.com', role: 'Senior Recruiter' },
    ...
    const leaderboard = roster.map((r, rIdx) => {
      ...
      const kpiScore = (sourced * 2) + (screened * 4) + (submissions * 8) + (interviews * 15) + (placed * 30);
    ...
    const totalSubmissions = leaderboard.reduce((acc, r) => acc + r.submissions, 0);
    const totalSourced = leaderboard.reduce((acc, r) => acc + r.sourced, 0);
    const totalPlaced = leaderboard.reduce((acc, r) => acc + r.placed, 0);
    const avgConversion = leaderboard.length > 0 ? Math.round(leaderboard.reduce((acc, r) => acc + r.conversionRate, 0) / leaderboard.length) : 0;
...
app.post('/api/admin/recruiters', async (req, res) => {
  ...
  const cleanPassword = String(password || 'recruiter123').trim();
  ...
  try {
    ...
    // 1. Update memory / disk mock roster
    ...
    if (existingIdx >= 0) {
      ...
      diskRec = recruitersMock[existingIdx];
    } else {
      diskRec = {
        _id: 'rec-' + Date.now(),
        name: cleanName,
      ...
      recruitersMock.unshift(diskRec);
    }
    saveRecruitersToDisk();

    // 2. Update MongoDB Atlas if connected
    if (isMongoConnected) {
      try {
        const emailRegex = new RegExp('^' + cleanEmail.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&') + '$', 'i');
        ...
        console.warn('MongoDB recruiter write warning, disk was saved:', mErr.message);
      }
    }
    ...
    return res.status(201).json({
      success: true,
      recruiter: {
        id: diskRec._id || diskRec.id,
        name: diskRec.name,
        email: diskRec.email,
        role: diskRec.role,
        parentRecruiterName: diskRec.parentRecruiterName,
        refCode: diskRec.refCode,
        company: diskRec.company,
        isActive: diskRec.isActive
      }
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
...
const DOMAIN_TAXONOMY = {
  ...
  database_admin: ['oracle', 'dba', 'database administrator', 'pl/sql', 'postgresql', 'mysql', 'sql server', 'mongodb', 'cassandra', 'dynamodb', 'database migration', 'rac', 'performance tuning', 'golden gate'],
  cloud_devops: ['devops', 'cloud engineer', 'aws', 'azure', 'gcp', 'kubernetes', 'docker', 'terraform', 'ci/cd', 'sre', 'ansible', 'cloud architect'],
  cybersecurity: ['cyber security', 'cybersecurity', 'infosec', 'soc analyst', 'penetration testing', 'siem', 'incident response', 'vulnerability', 'cissp', 'cism', 'ceh', 'identity access', 'iam', 'okta', 'crowdstrike', 'splunk'],
  network_security: ['network security', 'network engineer', 'cisco', 'palo alto', 'firewall', 'routing', 'switching', 'vpn', 'asa'],
 ...
 */
function extractCandidateExperience(resumeText = '', summary = '', rawProfile = null, role = '') {
...
// In-memory match cache to make /api/recruiter/email-streams run in <15ms instead of 30+ seconds
...
function classifyTechnicalDomain(title = '', skills = [], text = '') {
  ...
  for (const [domain, keywords] of Object.entries(DOMAIN_TAXONOMY)) {
    ...
    for (const kw of keywords) {
      if (title.toLowerCase().includes(kw)) score += 8; // Role title keyword carries heaviest weight
...
app.get('/api/recruiter/email-streams', (req, res) => {
  ...
  const scopedCandidates = (candidatesStore || []).filter(c => {
    ...
    const hasAttachment = (
      (c.resumeText && String(c.resumeText).length > 100) ||
      (c.attachmentName && String(c.attachmentName).trim().length > 0) ||
      (c.file && typeof c.file === 'object') ||
      (c.resumeUrl && String(c.resumeUrl).trim().length > 0) ||
      (c.legalDocs?.resume?.storageUrl && c.legalDocs.resume.storageUrl.length > 0) ||
      (c.legalDocs?.resume?.hasFile === true) ||
      (c.documents?.resume?.storageUrl && c.documents.resume.storageUrl.length > 0)
    );
    if (!hasAttachment) return false; // Skip no-attachment candidates entirely
