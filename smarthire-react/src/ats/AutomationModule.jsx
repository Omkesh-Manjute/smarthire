    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: isOk ? '#dcfce7' : '#fef3c7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: isOk ? '#15803d' : '#b45309' }}>
            {icon}
          </div>
          <div>
...
function AutomationModule() {
  ...
  const [webhookLog] = useState([
    { time: '08:00:12', type: 'incoming', msg: 'Webhook POST /api/webhook/email from n8n (payload: 4.2KB)' },
    { time: '08:00:13', type: 'ai', msg: 'Groq llama-3.3-70b parsed resume: 14 skills extracted.' },
  ...
  const integrations = [
    { icon: 'WH', name: 'n8n Webhook', description: 'Email-to-ATS pipeline', status: apiOnline ? 'Connected' : 'Disconnected', statusClass: apiOnline ? 'pill-ok' : 'pill-warn', detail: apiOnline ? 'Receiving webhook events from email parser. Last ping: < 2 min ago.' : 'Webhook endpoint not reachable. Check n8n workflow status.', action: 'Test Connection', onAction: () => alert('Testing n8n webhook connection...') },
    { icon: 'AI', name: 'Groq AI Parser', description: 'Resume parsing & JD matching', status: 'Active', statusClass: 'pill-ok', detail: 'Groq LLM API connected. Processing resumes with llama-3.3-70b-versatile. Average parse time: 1.8s.' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h3 style={{ margin: 0, fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#0f172a', fontSize: 16 }}>Automation & Integrations</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        {[
          { label: 'Services Online', value: apiOnline ? '6/6' : '4/6', color: '#16a34a', bg: '#f0fdf4' },
          { label: 'Cron Jobs Active', value: '5', color: '#2563eb', bg: '#eff6ff' },
          { label: 'Webhooks Today', value: '28', color: '#d97706', bg: '#fffbeb' },
          { label: 'AI Calls Made', value: '142', color: '#7c3aed', bg: '#f5f3ff' },
        ].map(s => (
      ...
      <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
          <h4 style={{ margin: 0, fontSize: 12, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Scheduled Jobs</h4>
        </div>
